...
public class EditInfo extends VerticalPanel {
	 ...
	 * 
	 * @param profilId
	 *            Die Profil-ID des aktuellen Nutzerprofils.
	 * @param profiltyp
	 *            Der Profiltyp (Nutzerprofil / Suchprofil).
	 * @param listtyp
	 *            Der Listtyp der Seite, von der das Bearbeiten der Infos
	 *            aufgerufen wird (Nutzerprofil / Suchprofil).
	 */
	...
	public void run() {
		...
		/**
		 * Fall, profiltyp gehoert zu Nutzerprofil. Bei Klick auf dieses Label
		 * wird auf die Seite zum Anzeigen des Nutzerprofils geleitet.
		 */
		if (profiltyp.equals("Np")) {
			...
			/**
			 * Fall, profiltyp gehoert zu Suchprofil. Bei Klick auf dieses Label
			 * wird auf die Seite zum Anzeigen des Suchprofils geleitet.
			 */
		 ...
		 * Sobald dieser Button betaetigt wird, werden die Eingaben der Tabelle
		 ...
		 * Information ueber diesen Zustand ausgegeben. Diese Daten
		 * aktualisieren die entsprechenden Daten in der Datenbank.
		 */
			...
			public void onClick(ClickEvent event) {
				...
				for (int i = 2; i < editInfoFlexTable.getRowCount(); i++) {
					...
					if (w instanceof TextArea) {
						...
						/**
						 * Fall, die TextArea ist leer. Dann wird die
						 * entsprechende Info in der Datenbank geloescht.
						 */
						...
						/**
						 * Fall, der Wert der ListBox ist "Keine Auswahl". Dann
						 * wird die entsprechende Info in der Datenbank
						 * geloescht.
						 */
		...
		/**
		 * ClickHandler fuer den Button zum Auswaehlen aller angezeigten Infos
		 * erzeugen. Sobald dieser Button betaetigt wird, werden alle CheckBoxen
		 * der Tabelle ausgewaehlt.
		 */
		...
		/**
		 * ClickHandler fuer den Button zum Abbrechen des Editierens erzeugen.
		 * Sobald dieser Button betaetigt wird, wird die Seite zum Anzeigen des
		 * eigenen Nutzerprofils angezeigt.
		 */
		...
		/**
		 * ClickHandler fuer den Button zum Loeschen der ausgewaehlten Infos
		 * erzeugen. Sobald dieser Button betaetigt wird, werden die
		 * ausgewaehlten Infos geloescht. Wurden alle Infos geloescht, wird die
		 * Seite zum Anzeigen des eigenen Nutzerprofils angezeigt.
		 */
				...
				if (counter == editInfoFlexTable.getRowCount()) {
					...
					for (int k = 2; k < rowTable; k++) {
						...
						if (checked2 == true) {
							...
							/**
							 * Fall, es wurden alle Infos geloescht, dann wird
							 * auf die Seite zum Anzeigen des Nutzerprofils
							 * weitergeleitet.
							 */
					...
					/**
					 * Fall, dass keine CheckBox ausgewaehlt wurde. Dann wird
					 * eine entsprechende Information angezeigt.
					 */
					if (zaehler == 0) {
						Window.alert("Es wurde nichts ausgewählt.");
					}
	...
	/**
	 * Methode, die die Infos anhand der Profil-ID aus der Datenbank ausliest
	 * und die Tabelle befuellt.
	 */
	...
	/**
	 * Methode, die die Optionen der Auswahleigeschaften anhand der Liste aus
	 * Eigenschaften aus der Datenbank ausliest und in die Tabelle einfuegt.
	 */
	...
	/**
	 * Loescht eine bestimmte Info des aktuellen Nutzerprofils aus der
	 * Datenbank.
	 * 
	 * @param eigenschaftIdTable
	 *            Id der zu loeschenden Eigenschaft
	 */
	public void loescheInfo(int eigenschaftIdTable) {

		ClientsideSettings.getPartnerboerseAdministration().deleteOneInfoNeu(profilId, eigenschaftIdTable,
	...
	/**
	 * Methode, die die angelegten Infos eines Nutzerprofils in der Datenbank
	 * speichert.
	 * 
	 * @param infos
	 *            Liste mit Infos zu einem Nutzerprofil
	 */
	public void speichereInfo(List<Info> infos) {

		/**
		 * ClickHandler fuer den Button zum Speichern einer Info erzeugen.
		 * Sobald dieser Button betaetigt wird, werden die Eingaben als Info zu
		 * einem Nutzrprofil oder Suchprofil gespeichert.
		 */
		ClientsideSettings.getPartnerboerseAdministration().saveInfo(profilId, infos, new AsyncCallback<Void>() {
	...
	/**
	 * Methode, die alle Infos loescht, wenn die Checkboxen aller Infos in der
	 * Tabelle ausgewaehlt sind.
	 */

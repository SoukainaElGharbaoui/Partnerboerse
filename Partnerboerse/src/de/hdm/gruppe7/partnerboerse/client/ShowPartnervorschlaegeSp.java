...
public class ShowPartnervorschlaegeSp extends VerticalPanel {
	...
	private Label ueberschriftLabel2 = new Label();
	...
	public void run() {
		...
		/**
		 * ClickHandler fuer Button zum Anlegen eines Suchprofils erzeugen. Bei
		 * der Betaetigung dieses Buttons gelangt man auf die Seite zum Anlegen
		 * eines neuen Suchprofils.
		 */
		...
		/**
		 * Clickhandler fuer den Button zum Anzeigen der Partnervorschlaege
		 * erzeugen. Sobald dieser Button betaetigt wird, werden alle alle
		 * Partnervorschlaege anhand des gewaehlten Suchprofils, nach
		 * Aehnlichkeit geordnet, ausgegeben.
		 */
		anzeigenSpButton.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {

				/**
				 * Bei jeder Auswahl eines Suchprofils wird die Tabelle komplett
				 * geleert, damit diese mit den neuen Informationen befüllt
				 * werden kann.
				 */
				...
				/**
				 * Widgets den Panels hinzufuegen. Diese Widgets werden
				 * angezeigt, sobald die Partnervorschlaege angezeigt werden.
				 */
		...
		/**
		 * Widgets den Panels hinzufuegen. Diese Widgets werden angezeigt sobald
		 * man über die MenueBar das Feld "Partnervorschlaege anhand Suchprofil"
		 * auswaehlt.
		 */
	...
	/**
	 * Befuellen der Tabelle mit den Partnervorschlaegen geordnet nach
	 * Aehnlichkeit.
	 */
	public void erzeugeTabelleSp() {
		...
		partnervorschlaegeSpFlexTable.setText(0, 4, "Geschlecht");
		partnervorschlaegeSpFlexTable.setText(0, 5, "Anzeigen");
		...
		ueberschriftLabel2.setText("Diese Profile passen zu deinem Suchprofil " + "'" + spName + "':");
				...
				spName, new AsyncCallback<List<Nutzerprofil>>() {
					...
					public void onSuccess(List<Nutzerprofil> result) {
						...
						for (Nutzerprofil np : result) {
							...
							row++;
							partnervorschlaegeSpFlexTable.setText(row, 0, String.valueOf(np.getAehnlichkeit()) + "%");
							partnervorschlaegeSpFlexTable.setText(row, 1, np.getVorname());
							partnervorschlaegeSpFlexTable.setText(row, 2, np.getNachname());
							...
							partnervorschlaegeSpFlexTable.setText(row, 3, geburtsdatumString);
							partnervorschlaegeSpFlexTable.setText(row, 4, np.getGeschlecht());
							...
							partnervorschlaegeSpFlexTable.setWidget(row, 5, anzeigenButton);

							/**
							 * Der Clickhandler fuer den Azeigen-Button des
							 * Fremdprofils wird hinzugefuegt.
							 * 
